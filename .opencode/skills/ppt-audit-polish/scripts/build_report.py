...
def build_report(findings: dict) -> str:
    ...
    visual_validation = findings.get("visual_validation", {})
    if visual_validation:
        lines.append("## Visual Validation")
        lines.append("")
        for label, payload in visual_validation.items():
            status = payload.get("status", "unknown")
            image_count = len(payload.get("images", []))
            lines.append(f"- {label}: `{status}` with `{image_count}` rendered image(s)")
            if payload.get("pdf"):
                lines.append(f"  - PDF: `{payload['pdf']}`")
        lines.append("")
