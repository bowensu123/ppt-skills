...
def main() -> int:
    ...
    polished_render_manifest = artifacts_dir / "polished-render.json"
    ...
    polished_images_dir = artifacts_dir / "polished-images"
    ...
    if render_data["status"] != "rendered":
        report_data["dependency_notes"].append(
            f"Original slide rendering skipped: {render_data.get('reason', 'unknown')}. "
            f"{render_data.get('dependency_hint', '')}".strip()
        )
    report_data["visual_validation"] = {"original": render_data}
    ...
    _run("render_slides.py", "--input", str(polished_path), "--output-dir", str(polished_images_dir), "--manifest", str(polished_render_manifest))
    polished_render_data = json.loads(polished_render_manifest.read_text(encoding="utf-8"))
    report_data["visual_validation"]["polished"] = polished_render_data
    if polished_render_data["status"] != "rendered":
        report_data["dependency_notes"].append(
            f"Polished slide rendering skipped: {polished_render_data.get('reason', 'unknown')}. "
            f"{polished_render_data.get('dependency_hint', '')}".strip()
        )
